		...
		foreach($_loginObject['columns'] as $_column)
		{
			...
			switch($_column['data_prc'])
			{
				...
				case 'crypt':	##	Encrypt form data for validation
								$_generatedValue = CRYPT::ENCRYPT($_formData[ $_column['field'] ], CRYPT::CHECKSUM($_formData[ $_column['field'] ]), true);						
								break;

				case 'hash':	##	Hash form data for validation
				default:
								$_generatedValue = CRYPT::HASH512($_formData[ $_column['field'] ], CRYPT::CHECKSUM(CRYPT::HASH256($_formData[ $_column['field'] ])), ENCRYPTION_BASEKEY ) . CRYPT::CHECKSUM(CRYPT::HASH256($_formData[ $_column['field'] ]));					
